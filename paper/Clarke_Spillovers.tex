...
$\Cov[D(i,t),R^j(i,t)]>0 \ \ \forall \ \ jc<d$, given that $D(i,t)$---the treatment
...
estimate $\hat\alpha^{j+1}$ is equal to the preceding estimate $\hat\alpha^{j}$.  At
this point, we can conclude that units at a distance of at least $jc$ from the 
...
$\alpha$, we can conclude that $d=jc$, and thus correctly identify 
$R(i,t)=\mathbf{1}_{X_i\leq jc}$ in data.
...
Let's start by considering the estimation of (\ref{Seqn:cDD}), however completely
ignoring the effect of spillovers $R(i,t)$.  Thus, we re-write (\ref{Seqn:cDD}) as:
\[
Y(i,t)=\mu+\tau D(i,1) + \gamma R(i,1) + \delta t + \alpha D(i,t) + \nu(i,t).
\]

where from (\ref{Seqn:cDD}), $\nu(i,t)=R(i,t)+\varepsilon(i,t)$.

If we were to estimate $\hat\beta$, then $\mathbb{E}[\hat\beta]=\beta+
\frac{Cov[R(i,t),D(i,t)]}{Var[D(i,t)]}+
\frac{Cov[\varepsilon(i,t),D(i,t)]}{Var[D(i,t)]}$
